...
(defn to-key-vector
  [des to prevresult]
  (if (is-simple-set-variable? to)
    {:to [(parse-simple-set-variable des to)]}
    {:to
     (into []
           (flatten
...
(defn to-key
  ...
  (let [result nil
        ...
        result (cond (vector? to)
                     ...
                     (string? to)
                     {:to (into [] (tos/parse-to des [{:shell to}]))}
                     (map? to)
                     {:to (into [] (tos/parse-to des to))}
                     :else result)]
    result))
...
(defn parse-rule
  ...
  ([des from to]
   (let [result {}
         ...
         result (assoc result :to (:to (to-key des to)))
         ...
         result (if conditions/used-simlayers-config
                  (let [insert-simlayer conditions/used-simlayers-config
                        insert-simlayer (assoc insert-simlayer :from
                                               (froms/parse-from des (:from insert-simlayer)))
                        insert-simlayer (assoc insert-simlayer :to
                                               (into [] (concat (tos/parse-to des (:from insert-simlayer)) (:to result))))
                        cleanup-used-simlayers-config (conditions/cleanup-used-simlayers-config)]
                    [result insert-simlayer])
                  result)]
     result)))
           ...
           (flatten
            (for [rule rules]
              (let [[from to condition other-options] rule]
                (do
                  (assert (and (nn? from) (nn? to)) (str "invalid rule: " des ", <from> or <to> is nil"))
                  (cond (and (nil? other-options) (nil? condition)) (parse-rule des from to)
                        ...
                        (and (nn? condition) (nn? other-options)) (parse-rule des from to condition other-options)))))))}))
